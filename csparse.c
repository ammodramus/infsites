...
/* y = A*x */
int cs_axpy (const cs *A, const double *x, double *y)
{
    int p, j, n, *Ap, *Ai ;
    double *Ax ;
    if (!A || !x || !y) return (0) ;	    /* check inputs */
    n = A->n; // number of columns
   	Ap = A->p; // Column pointers, (or column indices, when in triplet form)
	Ai = A->i; // row indices
	Ax = A->x;
    for (j = 0 ; j < n ; j++) // for each column...
    {
		printf("j = %i\tAp[j] = %i\n",j,Ap[j]);
		for (p = Ap[j] ; p < Ap[j+1] ; p++) // for each column pointer
		{
			y[Ai[p]] = Ax[p] * x[j]; // PW: this is my change. changed += to =.
			// What is Ai[p]??
		}
    }
    return (1) ;
...
int cs_axpy2 (const cs *A, const double *x, double *y)
{
    int p, j, n, *Ap, *Ai ;
    double *Ax ;
    if (!A || !x || !y) return (0) ;	    /* check inputs */
